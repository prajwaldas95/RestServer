...
namespace comicsdb
{
	...
	{
		ComicDb db;
		db.push_back(fromJson(R"json({"title":"The Fantastic Four", "issue":1,"writer":"Stan Lee", "penciler":"Jack Kirby", "inker":"Sol Brodsky", "letterer" :"Artie Simek","colorist" :"Stan Goldberg"})json"));
		{
			Comic comic;
	...
	{
		session->close(restbed::NOT_ACCEPTABLE, msg,{ {"Content-Type","text/plain"},{"Content-Length", std::to_string(msg.size())} });
	}
	...
	bool validId(const SessionPtr& session, const ComicDb& db, std::size_t& id)
	{
		...
		if (request->has_path_parameter("id"))
		{
			...
			notAcceptable(session, "NOT Accpetable, id out of range");
		}
		else
		{
	...
	void deleteComic(const SessionPtr& session, ComicDb& db)
	{
		...
		{
			{
				std::unique_lock<std::mutex> loack(g_dbMutex);
				db[id] = Comic{};
			}
	...
	void updateComic(const SessionPtr& session, ComicDb & db)
	{
		...
		std::size_t length{};
		length = request-> get_header("Conten-Length", length);
		if (length == 0)
		{
		...
		session->fetch(
			...
			[&db, id](const SessionPtr& session, const restbed::Bytes& data)
			{
				...
				if (comic.title.empty())
				{
					notAcceptable(session, "Invalid JSON");
					return;
				}
				else {
